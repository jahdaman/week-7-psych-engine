...
class PlayState extends MusicBeatState
{
	...
	var johns:FlxTypedGroup<FlxSprite>; //from johns to stumtimejohn is the idenifiers for the bopping shit
	var steve:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				case 'tank': //the actual stage that loads
				...
				tankBop1.animation.addByPrefix('idle', 'fg tankhead far right', 24, true);
				tankBop1.animation.play('idle', true);
				...
				tankBop2.animation.addByPrefix('idle','fg tankhead 5', 24, true);
				tankBop2.animation.play('idle', true);
				...
				tankBop4.animation.addByPrefix('idle','fg tankhead 4', 24, true);
				tankBop4.animation.play('idle', true);
				...
				tankBop5.animation.addByPrefix('idle','fg tankman bobbin 3', 24, true);
				tankBop5.animation.play('idle', true);
				...
				tankBop6.animation.addByPrefix('idle','fg tankhead far right', 24, true);
				tankBop6.animation.play('idle', true);
		...
		add(dadGroup); //LUNAR GOT TROLLED LMAOOOOOOO
		add(gfGroup);
		add(boyfriendGroup);

		if(curStage == 'tank') //layering, it's mainly for like 2 but i just added the rest cause why not
			add(tankBop1);
			add(tankBop2);
			add(tankBop3);
			add(tankBop4);
		...
		if(gfVersion == null || gfVersion.length < 1) {
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil':
					gfVersion = 'gf-pixel';
				case 'tank':
					if (curSong == 'stress') //sets the gf version for stage
						{
		...
		switch(curStage)
		{
			...
			case 'schoolEvil':
				...
				case 'tank': //tankmen being killed code
				if (gf.hasGun) 
					{
						gf.y += -100;
						gf.x -= -70;
			...
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					case 'ugh':
						...
						wellWell.frames = Paths.getSparrowAtlas('characters/TANK_TALK_1','shared');
						...
						wellWell.animation.callback = function(idle, frameNumber:Int, frameIndex:Int)
							{
								...
								if (frameNumber == 1)
									{
										camFollow.x = 290;
										FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom = 1.1}, 2);
										var sound:FlxSound;	
					...
					case 'guns':		
						trace("the most simple cutscene");
						dad.alpha = 0;
						healthBarBG.alpha = 0;
						healthBar.alpha = 0;
						iconP1.alpha = 0;
						...
						anim2.frames = Paths.getSparrowAtlas('characters/TANK_TALK_2','shared');
					...
					case 'stress':
						trace('i leanred how to fix it from the danvoltz ver');
						gf.alpha = 0.00001;
						...
						var gfDemon:FlxSprite = new FlxSprite(gf.x, gf.y);
						gfDemon.frames = Paths.getSparrowAtlas('characters/gfDemon');
						gfDemon.animation.addByPrefix('dance', 'GF Turnin Demon W Effect', 24);
						...
						var picoKill:FlxSprite = new FlxSprite(-600, -220);
						picoKill.frames = Paths.getSparrowAtlas('characters/pico_saves_them');
						picoKill.animation.addByPrefix('dance', 'pico_saves_them idle', 24); //sprite sheet generator lmao
						picoKill.antialiasing = true;
						picoKill.alpha = 0.00001;
						gfCutsceneLayer.add(picoKill);

						var picoSpeaker:FlxSprite = new FlxSprite(gf.x, gf.y);
						picoSpeaker.frames = Paths.getSparrowAtlas('characters/picoSpeaker');
						picoSpeaker.animation.addByIndices('dance', 'Pico shoot 1', [23, 24, 25], "", 24, true); //adding this for layering issues, this is very annoying
						picoSpeaker.antialiasing = true;
						picoSpeaker.alpha = 0.00001;
						gfCutsceneLayer.add(picoSpeaker); 

						var godeffing:FlxSprite = new FlxSprite(-100, 300);
						godeffing.frames = Paths.getSparrowAtlas('characters/TANK_TALK_3');
						godeffing.animation.addByPrefix('dance', 'TANK_TALK_3 idle', 24); //sprite sheet generator lmao
						godeffing.animation.play('dance');
						godeffing.antialiasing = true;
						godeffing.alpha = 1.00001;
						gfCutsceneLayer.add(godeffing);

						var bf:FlxSprite = new FlxSprite(boyfriend.x, boyfriend.y);
						bf.frames = Paths.getSparrowAtlas('characters/BOYFRIEND');
						bf.animation.addByPrefix('dance', 'BF idle dance', 24);
						bf.animation.play('dance');
						bf.antialiasing = true;
						bf.alpha = 1.00001;
						gfCutsceneLayer.add(bf);

						camFollowPos.setPosition(550, 450);
				
						
						new FlxTimer().start(0.1, function(GodEfftingDamnit:FlxTimer)
							{
								FlxG.sound.play(Paths.sound('cuts3'));
								gf.dance();
							});
						new FlxTimer().start(14.8, function(dagfDemon:FlxTimer)
							{
								camFollowPos.setPosition(550, 300);
								...
								gfCutsceneLayer.add(gfDemon);
								gfDemon.animation.play('dance');
							});
						new FlxTimer().start(17.2, function(demonpart2:FlxTimer)
							{
								FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom = 1.0}, 2);
								gfCutsceneLayer.remove(bf);
								gfCutsceneLayer.remove(gfDemon);
								picoKill.alpha = 1;
								gfCutsceneLayer.add(picoKill);
								picoKill.animation.play('dance');
								boyfriend.alpha = 1;
						...
						new FlxTimer().start(20.1, function(demonpart2:FlxTimer)	
							{
								camFollowPos.setPosition(550, 450);
							});							
						new FlxTimer().start(20.5, function(dagfDemon:FlxTimer)
							{
								gfCutsceneLayer.remove(picoKill);
								gfCutsceneLayer.add(picoSpeaker);
								picoSpeaker.alpha = 1;
								picoSpeaker.animation.play('dance');
							});			
						...
						new FlxTimer().start(35.1, function(eugh:FlxTimer)	
							{
								dad.visible = true;
								gfCutsceneLayer.remove(godeffing);
								gfCutsceneLayer.remove(picoSpeaker);
								gf.alpha = 1;
							});
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'mall':
				...
				case 'tank': //moves steve in the bg
				moveTank();
				var i = 0; //resets tankmen being killed
		...
		if(!endingSong && !startingSong) {
			...
			if(FlxG.keys.justPressed.F2) { //Go 10 seconds into the future :O
	...
	var tankX = 400;
	function moveTank() { //function for the tank moving
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'roadkill_enthusiast':
						if(Achievements.henchmenDeath >= 1) {
							unlock = true;
						}
					case 'oversinging':
